...
(*----------------------------------------------------------------------------*
 ...
 zaporedja in vrne seznam vseh členov, dokler zapore
 dje ne doseže $1$.
[*----------------------------------------------------------------------------*)
let rec collatz n =
  if n = 1 then [1]
  else if n mod 2 = 0 then n :: collatz (n/2)
...
let sep_concat loc sez =
  List.flatten (List.map (fun x -> loc :: x) sez) @ [loc]
...
let rec partition k sez = 
  let rec aux k sez acc kterice =
    match sez with
    | [] -> (
      if kterice = [] then List.rev acc
      else List.rev (List.rev (kterice) :: acc)
    )
    | x :: xsez -> (
      if List.length kterice < k then aux k xsez acc (x :: kterice)
      else
        aux k xsez (List.rev kterice :: acc) [x]
    ) in
  aux k sez [] []
...
let inverz p = 
  let inv = List.sort (fun (x, _) (y, _) -> compare x y) (List.mapi (fun i x -> (x, i)) p) in
  List.map (fun (x,y) -> y) inv
...
let cikli p = 
  let pari = List.mapi (fun i x -> (i, x)) p in

  let rec poisci_sliko i perm =
    match perm with
    | [] -> failwith "you never saw this" (*itak ne zgodi*)
    | (x, y) :: x_ysez -> (
      if x = i then y else poisci_sliko i x_ysez
    ) in
  
  let rec prvi_cikel p trenutni acc =
    let y = poisci_sliko trenutni p in
    if List.mem y acc then List.rev (acc)
    else prvi_cikel p y (y :: acc) (*za akumulator ne bom vzel [] ampak [zacetek]*)
  in

  let rec odstrani p cikel =
    List.filter (fun (a, _) -> not (List.mem a cikel)) p
  in
  
  let rec zgradi_cikle p acc =
    match p with
    | [] -> List.rev acc
    | (i, _) :: xsez -> (
      let c = prvi_cikel p i [i] in
      let perm_brez = odstrani p c in
      zgradi_cikle perm_brez (c :: acc)
    ) in
  zgradi_cikle pari []
...
let iz_transpozicij n trans = 
  let p = List.init n (fun x -> x) in
  let swap sez (i, j) =
    List.mapi (fun index x -> 
      if index = i then List.nth sez j
      else if index = j then List.nth sez i
      else x
      ) sez in

  List.fold_left swap p trans
...
let v_transpozicije p = 
  let c = cikli p in
  let trans = List.map (fun cik -> List.rev_map (fun x -> (List.nth cik 0, x)) cik) c in
  List.filter (fun (x,y) -> x != y) (List.flatten trans)
...
let dodaj i j n m = 
  Array.init 9 (fun vrstica -> Array.init 9 (fun stolpec -> if vrstica = i && stolpec = j then Some n else m.(vrstica).(stolpec)))
...
let izpis_mreze m = 
  let pretvori_v_string el = 
    match el with
    | None -> "."
    | Some n -> string_of_int n
  in

  let rec vrstica_to_string j vrstica =
    match vrstica with
    | [] -> "\n"
    | x :: xsez -> (
      if j mod 3 = 0 then pretvori_v_string x ^ " | " ^ vrstica_to_string (j + 1) xsez
      else pretvori_v_string x ^ " " ^ vrstica_to_string (j + 1) xsez
      )
  in

  let rec vse_vrstice i mreza =
    match mreza with
    | [] -> "+-------+-------+-------+\n"
    | x :: xmreza -> (
      if i mod 3 = 0 then "+-------+-------+-------+\n" ^ "| " ^ vrstica_to_string 1 (Array.to_list x) ^ vse_vrstice (i + 1) xmreza
      else "| " ^ vrstica_to_string 1 (Array.to_list x) ^ vse_vrstice (i + 1) xmreza
    )
    in

    vse_vrstice 0 (Array.to_list m)
...
let izpis_resitve resitev = 
  let pretvori_v_string n = string_of_int n in
  ...
  let rec vse_vrstice i mreza =
    ...
    vse_vrstice 0 (Array.to_list resitev)
...
let ustreza mreza resitev = 
  Array.for_all2 (fun vrstica_m vrstica_r -> (
    Array.for_all2 (fun el_m el_r -> (
      match el_m with
      | None -> true
      | Some n -> n = el_r
    )) vrstica_m vrstica_r
...
let ni_v_vrstici mreza_i n  = 
  Array.for_all (fun el -> 
    match el with
    | None -> true
    | Some el -> el != n
  ) (fst mreza_i).(snd mreza_i)
...
let ni_v_stolpci mreza_j n  = 
  Array.for_all (fun vrstica ->
    match vrstica.(snd mreza_j) with
    | None -> true
    | Some el -> el != n
    ) (fst mreza_j)

let ni_v_skatli mreza_k n  = 
  let prva_vrstica = (snd (mreza_k) / 3) * 3 in
  let prvi_stolpec = (snd (mreza_k) mod 3) * 3 in
  Array.for_all (fun vrstica -> 
    Array.for_all (fun stolpec -> 
      match (fst mreza_k).(vrstica).(stolpec) with
      | None -> true
      | Some el -> n != el
      )[|prvi_stolpec; prvi_stolpec + 1; prvi_stolpec + 2|])
      [|prva_vrstica; prva_vrstica + 1; prva_vrstica + 2|]
...
let kandidati mreza i j = 
  ...
  | None -> Some (
    List.filter (fun x -> 
      ni_v_skatli (mreza, skatla) x && 
      ni_v_vrstici (mreza, i) x && 
      ni_v_stolpci (mreza, j) x) [1;2;3;4;5;6;7;8;9]
  )
...
let rec resi mreza = 
  let rec najmanj_kandidatov i j naj_par naj_dolzina =
    if i = 9 then naj_par
    else if j = 9 then najmanj_kandidatov (i + 1) 0 naj_par naj_dolzina
    else 
      match mreza.(i).(j) with
      | Some n -> najmanj_kandidatov i (j + 1) naj_par naj_dolzina
      | None -> (
        let kand = kandidati mreza i j in
        match kand with 
        | None -> najmanj_kandidatov i (j+1) naj_par naj_dolzina (*se itak ne zgodi ampak ocitno je treba obravnavat*)
        | Some [] -> Some (-1,-1) (*neresljiv primer*)
        | Some sez -> (
          let len = List.length sez in
          if len < naj_dolzina then najmanj_kandidatov i (j + 1) (Some (i,j)) len
          else najmanj_kandidatov i (j + 1) naj_par naj_dolzina
        )
      ) in


  match najmanj_kandidatov 0 0 None 10 with
  | None -> Some (Array.map (Array.map (
    function 
    | Some x -> x
    | None -> 0
  )) mreza) (*baza*)
  ...
  | Some (i, j) -> (
      ...
      | Some sez -> (
          let rec poskus sez =
            ...
            | [] -> None
            | x :: xsez -> (
              let nova = dodaj i j x mreza in
              match resi nova with
